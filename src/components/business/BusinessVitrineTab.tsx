...
export const BusinessVitrineTab = ({ businessId, businessName }: BusinessVitrineTabProps) => {
  ...
  const convertCatalogsToProducts = (catalogs: Catalog[]) => {
    return catalogs.flatMap(catalog => {
      ...
      // If it's a product catalog, treat each image as a separate product
      if (catalog.catalog_type === 'products' && images.length > 1) {
        return images.map((image, index) => ({
          id: `${catalog.id}-${index}`,
          name: `${catalog.name} - Article ${index + 1}`,
          ...
          tags: Array.isArray(catalog.keywords) ? catalog.keywords as string[] : [],
          rating: Math.floor(Math.random() * 2) + 4, // 4-5 stars
          reviews_count: Math.floor(Math.random() * 50) + 1,
          delivery_info: catalog.delivery_available ? {
            available: true,
            cost: catalog.delivery_cost,
            zones: Array.isArray(catalog.delivery_zones) ? catalog.delivery_zones as string[] : [],
        ...
        return [{
          ...
          condition: 'new' as const,
          availability: 'in_stock' as const,
          stock_quantity: catalog.catalog_type === 'services' ? undefined : Math.floor(Math.random() * 20) + 1,
          tags: Array.isArray(catalog.keywords) ? catalog.keywords as string[] : [],
          ...
          delivery_info: catalog.delivery_available ? {
            ...
            cost: catalog.delivery_cost,
            zones: Array.isArray(catalog.delivery_zones) ? catalog.delivery_zones as string[] : [],
            estimated_time: catalog.catalog_type === 'services' ? "Sur rendez-vous" : "24-48h"
          } : undefined
        }];
      }
    });
  ...
  const products = convertCatalogsToProducts(catalogs);
    ...
    <div className="space-y-6 pb-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            {products.length} articles • {catalogs.length} catalogues
          </p>
        </div>
