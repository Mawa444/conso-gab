...
const businessCategories = [
  { id: "restaurant" as const, name: "Restaurant" },
  { id: "retail" as const, name: "Commerce" },
  { id: "services" as const, name: "Services" },
  { id: "technology" as const, name: "Technologie" },
  { id: "healthcare" as const, name: "Santé" },
  { id: "education" as const, name: "Éducation" },
  { id: "finance" as const, name: "Finance" },
  { id: "real_estate" as const, name: "Immobilier" },
  { id: "automotive" as const, name: "Automobile" },
  { id: "beauty" as const, name: "Beauté" },
  { id: "fitness" as const, name: "Fitness" },
  { id: "entertainment" as const, name: "Divertissement" },
  { id: "agriculture" as const, name: "Agriculture" },
  { id: "manufacturing" as const, name: "Industrie" },
  { id: "other" as const, name: "Autres" }
];
...
export const UnifiedSearchBar = ({
  ...
  const { search, results, loading, error, clearResults } = useUnifiedSearch();

  useEffect(() => {
    if (debouncedQuery || Object.values(filters).some(f => 
      f !== defaultFilters[f as keyof UnifiedSearchFilters]
    )) {
      ...
      <div className="flex items-center gap-2">
        ...
        {onVoiceSearch && (
          <Button
            ...
            className={cn(sizeClasses[size], "px-3", isListening ? 'bg-red-50 border-red-200' : '')}
          >
        ...
        <div className="flex flex-wrap gap-1 pt-3 border-t">
          ...
          {filters.location && (
            <Badge
              variant="secondary"
              className="text-xs cursor-pointer hover:bg-destructive/10 hover:text-destructive"
              onClick={() => toggleFilter('location', undefined)}
            >
          ...
          <CardContent className="p-4">
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {currentLocation && (
                  <Button
                    variant={filters.location === currentLocation ? "default" : "outline"}
                    size="sm"
                    onClick={() => toggleFilter('location', currentLocation)}
                    className="h-8 text-xs"
                  >
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {results.map((result) => (
                  <div
                    ...
                    className="p-3 border border-border/30 rounded-lg hover:border-primary/30 hover:bg-primary/5 cursor-pointer transition-all duration-200 flex items-center gap-3"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-xs">
                          {result.type === 'business' ? 'Entreprise' : 
                           result.type === 'catalog' ? 'Catalogue' : 'Produit'}
                        </Badge>
                      </div>
