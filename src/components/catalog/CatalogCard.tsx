...
export const CatalogCard = ({ 
  ...
  return (
    <>
      <div 
        className="bg-card rounded-2xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] border border-border/20 transition-all duration-300 hover:shadow-[0_8px_24px_rgba(0,0,0,0.16)] hover:scale-[1.01] cursor-pointer overflow-hidden"
          ...
          <div className="relative h-48 bg-gradient-to-br from-muted to-muted/50 overflow-hidden">
            ...
            <div className="absolute top-3 left-3 flex flex-wrap gap-2">
              {catalog.catalog_type === 'services' && (
                <Badge variant="secondary" className="text-xs px-2 py-1 bg-blue-500/90 text-white font-medium">
                  Services
                </Badge>
              )}
              {catalog.on_sale && catalog.sale_percentage && catalog.sale_percentage > 0 && (
                <Badge variant="secondary" className="text-xs px-2 py-1 bg-red-500/90 text-white font-medium flex items-center gap-1">
                  <Percent className="w-3 h-3" />
                  -{catalog.sale_percentage}%
                </Badge>
              )}
              {catalog.delivery_available && (
                <Badge variant="secondary" className="text-xs px-2 py-1 bg-green-500/90 text-white font-medium flex items-center gap-1">
                  <Truck className="w-3 h-3" />
                  Livraison
                </Badge>
              )}
              {catalog.has_limited_quantity && (
                <Badge variant="secondary" className="text-xs px-2 py-1 bg-orange-500/90 text-white font-medium">
                  Quantité limitée
                </Badge>
              )}
            </div>
            ...
            {isOwner && (
              <div className="absolute top-3 right-3 flex gap-2">
                ...
                <Button
                  variant="secondary"
                  size="sm"
                  className="p-2 h-8 w-8 bg-white/90 hover:bg-white text-red-600 hover:text-red-700"
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              {catalog.is_public && (
                <Badge variant="outline" className="text-xs px-2 py-1 border-green-500 text-green-600 bg-green-50 font-medium">
                  <Shield className="w-3 h-3 mr-1" />
                  Public
                </Badge>
              )}
              {!catalog.is_public && (
                <Badge variant="outline" className="text-xs px-2 py-1 border-gray-400 text-gray-600 bg-gray-50 font-medium">
                  Privé
                </Badge>
              )}
            </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Button
                variant="ghost"
                size="sm"
                className={cn("p-2 h-11 w-11", isLiked && "bg-red-50 text-red-600 hover:bg-red-100")}
                onClick={handleLike}
              >
