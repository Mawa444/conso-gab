                  ...
                  {categoryStats.slice(0, 10).map(stat => (
                    <div 
                      ...
                      className={`p-2 rounded-lg cursor-pointer transition-colors border ${
                        selectedCategory === stat.id ? 'bg-primary/10 border-primary/30' : 'hover:bg-muted/50 border-transparent'
                      }`}
            ...
            <div className="grid gap-4">
              {filteredAndSortedBusinesses.map(business => (
                <RealBusinessCard 
                  key={business.id} 
                  business={business} 
                  onSelect={() => navigate(`/business/${business.id}`)} 
                  onMessage={() => console.log("Message to", business.name)} 
                  variant="default" 
                />
              ))}
