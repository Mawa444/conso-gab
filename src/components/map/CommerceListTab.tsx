...
export const CommerceListTab = () => {
  ...
  const filteredAndSortedCommerces = useMemo(() => {
    let filtered = allCommerces.filter(commerce => {
      ...
      const searchMatch = searchQuery === "" || commerce.name.toLowerCase().includes(searchQuery.toLowerCase()) || commerce.type.toLowerCase().includes(searchQuery.toLowerCase()) || commerce.category.toLowerCase().includes(searchQuery.toLowerCase()) || commerce.district.toLowerCase().includes(searchQuery.toLowerCase()) || commerce.specialties?.some(s => s.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      return searchMatch && categoryMatch && districtMatch && distanceMatch && ratingMatch && priceMatch && openMatch && verifiedMatch;
    });
    ...
    switch (sortBy) {
      ...
      case "price_low":
        filtered.sort((a, b) => {
          ...
          return (priceOrder[a.priceRange as keyof typeof priceOrder] || 2) - (priceOrder[b.priceRange as keyof typeof priceOrder] || 2);
        });
        break;
      case "price_high":
        filtered.sort((a, b) => {
          ...
          return (priceOrder[b.priceRange as keyof typeof priceOrder] || 2) - (priceOrder[a.priceRange as keyof typeof priceOrder] || 2);
        });
        break;
      case "employees":
  ...
  const districtStats = useMemo(() => {
    const stats = districts.map(district => {
      ...
      return {
        ...
        avgRating: districtCommerces.length > 0 ? (districtCommerces.reduce((sum, c) => sum + c.rating, 0) / districtCommerces.length).toFixed(1) : "0",
      ...
      <div className="bg-card/95 backdrop-blur-sm border-b border-border/50 p-4 space-y-4">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1 rounded-3xl">
            ...
            <Input placeholder="Rechercher par nom, type, spécialité, quartier..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 h-11" />
          </div>
            ...
            <CardContent className="p-4 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Slider value={distanceRange} onValueChange={setDistanceRange} max={20} min={1} step={0.5} className="w-full" />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    {["€", "€€", "€€€"].map(price => <Button key={price} variant={priceFilter.includes(price) ? "default" : "outline"} size="sm" onClick={() => togglePriceFilter(price)} className="flex-1">
                        {price}
                      </Button>)}
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 p-4">
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {districtStats.slice(0, 10).map(stat => <div key={stat.name} className={`p-2 rounded-lg cursor-pointer transition-colors border ${selectedDistrict === stat.name ? 'bg-primary/10 border-primary/30' : 'hover:bg-muted/50 border-transparent'}`} onClick={() => setSelectedDistrict(selectedDistrict === stat.name ? "all" : stat.name)}>
                ...
                <div className="grid grid-cols-2 gap-4 text-center">
                  ...
                  <div className="p-3 bg-orange-500/5 rounded-lg">
                    <div className="text-lg font-bold text-orange-600">
                      {filteredAndSortedCommerces.length > 0 ? (filteredAndSortedCommerces.reduce((acc, c) => acc + c.rating, 0) / filteredAndSortedCommerces.length).toFixed(1) : '0'}
                    </div>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  ...
                  {selectedDistrict !== "all" && <Badge variant="secondary">{selectedDistrict}</Badge>}
            ...
            <div className="grid gap-4">
              {filteredAndSortedCommerces.map(commerce => <CommerceCard key={commerce.id} commerce={commerce} onSelect={() => navigate(`/business/${commerce.id}`)} onMessage={() => console.log("Message to", commerce.name)} variant="default" />)}
              
              {filteredAndSortedCommerces.length === 0 && <div className="text-center py-12 text-muted-foreground">
